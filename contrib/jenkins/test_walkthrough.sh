...
[[ "$(kubectl get serviceclasses)" == *${USER_PROVIDED_SERVICE_ID}* ]] \
...
kubectl create -f "${ROOT}/contrib/examples/walkthrough/ups-instance-credential.yaml" \
...
[[ "$(kubectl get serviceinstancecredentials -n test-ns ups-instance-credential -o yaml)" == *"status: \"True\""* ]] \
  || {
    kubectl get serviceinstancecredentials -n test-ns ups-instance-credential -o yaml
